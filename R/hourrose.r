...
#'
#' @param my_df A data frame as the input data
#'
#' @return Parameters prepared for the grid.
#'
#' @examples prepare.grid(NA)
prepare.grid <- function(my_df) {
  if (all(is.na(my_df$x)))
    return()
  levels(my_df$x) <- paste0("x", 1:length(theLabels))
  all <- stat.fun(my_df[, wd])
  calm <- my_df[my_df[, wd] == -999, ][, pollutant]
  my_df <- my_df[my_df[, wd] != -999, ]
  calm <- stat.fun(calm)
  weights <- tapply(my_df[, pollutant],
                    list(my_df[, wd], my_df$x), stat.fun)
  if (stat.scale == "all") {
    calm <- calm/all
    weights <- weights/all
  }
...
#' The panel function for the hour rose plot
#'
#' @param x
#' @param y
#' @param subscripts
...
#'
#' @examples
panel_hourrose <- function(x, y, subscripts, ...) {
  panel.xyplot(x, y, ...)
  angles <- seq(0, 2 * pi, length = 360)
  sapply(seq(off.set, mymax, by = myby),
         function(x) llines(x * sin(angles),
                            x * cos(angles),
                            col = "grey85",
                            lwd = 1))
  ...
  lar <- upper * 0.9
  for (langle in seq(0, 2 * pi, pi/4)) {
    larrows(0, 0, lar * cos(langle), lar * (sin(langle)),
            code = 3, length = 0, col = 'grey')
  }
  for (langle in seq(0, 2 * pi, pi/2)) {
    larrows(0, 0, lar * cos(langle), lar * (sin(langle)),
            code = 3,length = 0)
  }
  if (quantile_line) {
    for (langle in quantile(my_df[, wd], seq(0.25, 1, 0.25)) / 180 * pi) {
      larrows(0, 0, lar * sin(langle), lar * (cos(langle)),
  ...
  if (annotate) if (statistic != "prop.mean") {
    ...
    ltext(max.freq + off.set, -max.freq - off.set,
          label = paste0(stat.lab2, " = ", subdata$panel.fun[1],
                        stat.unit), adj = c(1, 0), cex = 0.7,
          col = calm.col)
  }
}

#' Poly function when paddle == TRUE
#'
#' @param wd
#' @param len1
#' @param len2
...
#'
#' @examples
poly_paddle <- function(wd, len1, len2, width, colour,
                        x.off = 0,
                        y.off = 0) {
  theta <- wd * pi/180
  len1 <- len1 + off.set
  ...
  lpolygon(c(x1, x2, x4, x3), c(y1, y2, y4, y3),
           col = colour, border = border)
}

#' Poly function when paddle == FALSE
#'
#' @param wd
#' @param len1
#' @param len2
...
#'
#' @examples
poly_paddless <- function(wd, len1, len2, width, colour,
                          x.off = 0,
                          y.off = 0) {
  len1 <- len1 + off.set
  len2 <- len2 + off.set
                     ...
                     ...)
{
  ...
  xyplot.args <- list(x = myform,
                      ...
                      scales = list(draw = FALSE), panel = panel_hourrose,
                      legend = legend)
